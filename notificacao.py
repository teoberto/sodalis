...
from helper import ConectaBancoDados
...
def enviar_notificacao_diaria():
    ...
    for nr_whatsapp, dados in usuarios_tarefas.items():
        ...
        prompt_tema = ConectaBancoDados().executar_query(
            """
            SELECT 'Tema: ' || prompt as prompt FROM public.texto_ia 
            WHERE id_texto_ia = (SELECT id_texto_ia FROM public.texto_ia_usuario WHERE id_usuario = :id_usuario)
            """,
            {"id_usuario": id_usuario}
        )

        prompt_tema = prompt_tema[0]['prompt']
        ...
        motivacional = generate_response(
            ...
            system_prompt=f"Tema: {prompt_tema}"
        )
...
def consultar_tarefas_diarias():
    banco = ConectaBancoDados()
    return banco.executar_query(
        """SELECT usuario.id_usuario,usuario.nr_whatsapp, usuario.nome, tarefa.nm_tarefa
            FROM atribuicao
